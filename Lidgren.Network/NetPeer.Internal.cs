	...
	public partial class NetPeer
	{
				...
				lock (m_connections)
				{
					foreach (NetConnection conn in m_connections)
					{
						...
						if (conn.m_status == NetConnectionStatus.Disconnected)
						{
							...
							// remove connection
							//
							LogWarning("heartbeat removing disconnected " + conn.RemoteEndPoint);
		...
		private void ReceivedUnconnectedLibraryMessage(double now, IPEndPoint senderEndPoint, NetMessageType tp, int ptr, int payloadByteLength)
		{
			...
			switch (tp)
			{
				...
				case NetMessageType.ConnectResponse:
					...
					LogWarning("Received unhandled library message " + tp + " from " + senderEndPoint + ". handshakes: " + m_handshakes.Count);
					return;
		...
		internal void AcceptConnection(NetConnection conn)
		{
			...
			lock (m_connections)
			{
				...
				{
					LogWarning("[AcceptConnection] Adding connection: " + conn.m_remoteEndPoint);
