using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using Analytics.Global;

namespace Analytics.Gameplay
{
    public class MissionStarted
    {
        public string eventName = "missionStarted";
        public string userID = "";
        public string sessionID = "";
        public string eventTimestamp = DateTime.Now.ToString();
        public EventParams_MissionStarted eventParams = new EventParams_MissionStarted();
        public GoalCounts goalCounts = new GoalCounts();

        public MissionStarted()
        { }

        public MissionStarted(string i_userID, string i_sessionID, string i_eventTimestamp)
        {
            userID = i_userID; 
            sessionID = i_sessionID;
            eventTimestamp = i_eventTimestamp; 
        }
  }
   
    public class EventParams_MissionStarted
    {
        public string missionName = "";
        public string missionID = "";
        public bool isTutorial = true;

        public EventParams_MissionStarted()
        { }

        public EventParams_MissionStarted(string i_missionName, string i_missionID, bool i_isTutorial)
        {
            missionName = i_missionName;
            missionID = i_missionID;
            isTutorial = i_isTutorial;
        }
    }
/*
            Example:
            {
                "eventName": "missionStarted",
                "userID": " GA_098987AY127FAFS1192",
                "sessionID": "f32f4a04-e75b-42f0-a2a9-18c0805f09b1",
                "eventParams": {
                    "missionName": "In to the Valley of Death",
                    "missionId": "25412514",
                    "isTutorial ": false
                },
                "goalCounts":{
                    "userLevel": 10, 
                    "experience": 10065,
                    "health": 1213, 
                    "energy": 1213, 
                    "strength": 6, 
                    "dexterity": 7, 
                    "focus": 12, 
                    "vitality": 13, 
                    "attack": 53, 
                    "defence": 12, 
                    "damage": 70, 
                    "armour": 15, 
                    "gold": 500, 
                    "platinum": 25
                }
            } 

         */
  
}
