...
namespace MainServer
{
    class ASPathFinder
    {
        ...
        DateTime m_StopPathingTime = DateTime.Now;
        int m_maxIterationsPerFrame = 30;
        int m_maxClosedList = 50;
        int m_currentIterationsThisFrame = 0;

        internal DateTime StopPathingTime
        {
            set { m_StopPathingTime = value; }
        }
        ...
        internal ASPathFinder(string filename)
        {
            m_theMap = new ASMap(filename);
        ...
        {
            //pather.Reset();
            m_currentIterationsThisFrame = 0;
        }

        /// <summary>
        /// Returns a path of points from start point to end point
        /// </summary>
        /// <param name="pathingObject"></param>
        /// <returns>The object looking for a path</returns>
        internal List<Vector3> GetPath(ASPathingEntity pathingObject)
        {
            if (pathingObject == null)
            {
                Console.WriteLine("null pathing Object passed in to Get Path");
                return null;
            }
            DateTime currentFunctionTime = DateTime.Now;

            if (currentFunctionTime >= m_StopPathingTime)
            {
                pathingObject.LastError = AS_PATHING_ERROR.PATHING_LIMIT_EXCEEDED;
                return null;
            }
            ...
            ASNode endNode = pathingObject.EndNode;
            ...
            {
                List<Vector3> basicPath = new List<Vector3>();
                //basicPath.Add(startPoint);
                basicPath.Add(endPoint);
            ...
            int iterationAllowance = m_maxIterationsPerFrame;
            int currentIteration = m_currentIterationsThisFrame;
            ...
            while (pathFound==false &&
                ...
                endNode==null&&
                currentFunctionTime < m_StopPathingTime&&
                closedList.Count<m_maxClosedList
                )
                {
                    currentIteration++;
                ...
                //for all links
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        ASNode newNode = new ASNode(currentLink, currentNode, cost, heuristic, lineEntry);//adjoiningLine.MidPoint);
                    ...
                    //add it to the start of the closed list as it is likely to be retested soon
                    closedList.Insert(0, currentNode);
                    openList.Remove(currentNode);
                    currentFunctionTime = DateTime.Now;
            //go to next
            }
            //if the closed list is to big or the open list has been emptied there is no viable path to this position
            if(((closedList.Count>=m_maxClosedList)||(openList.Count<=0))&&(pathFound==false))
            {
                pathingObject.LastError = AS_PATHING_ERROR.NO_PATH;
                return null; 
            }
            m_currentIterationsThisFrame = currentIteration;
            //if too much time has been taken don't optimise till next frame
            if (currentFunctionTime >= m_StopPathingTime)
            {
                pathingObject.LastError = AS_PATHING_ERROR.PATHING_LIMIT_EXCEEDED;
                return null;
        ...
        List<ASNode> GetShortestNodeListTwoSidedTo(List<ASNode> fullNodeList, Vector3 startPoint, Vector3 endPoint)
        {
            ...
            if(endMidNode!=startMidNode&&(endMidNode!=null))
            {
